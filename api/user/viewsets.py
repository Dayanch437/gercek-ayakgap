...
@extend_schema(tags=["user"], description="", responses=UserSerializer)
...
@extend_schema(
    ...
    responses={
        200: {
            ...
            "examples": {
                "application/json": {"message": "Verification code sent to your email"}
            },
        },
        400: {
            "description": "Invalid request",
            "examples": {"application/json": {"error": "Invalid email format"}},
        },
        404: {
            "description": "User not found",
            "examples": {
                "application/json": {"error": "No user with this email address"}
            },
        },
    },
    methods=["POST"],
...
def request_password_reset(request):
    ...
    return Response(
        {"message": "Verification code sent to your email"}, status=status.HTTP_200_OK
    )
...
def verify_otp(request):
    ...
    if not otp_obj.is_valid():
        return Response(
            {"error": "OTP expired or already used"}, status=status.HTTP_400_BAD_REQUEST
        )
